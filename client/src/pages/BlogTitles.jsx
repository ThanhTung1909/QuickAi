...
const BlogTitles = () => {
  ...
  const [selectedCategry, setSelectedCategory] = useState("General");
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      in the category ${selectedCategry}`;
  ...
  return (
    <div className="h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700">
        ...
        <div className="mt-3 flex gap-3 flex-wrap sm:max-w-9/12">
          {blogCategories.map((item, index) => (
            <span
              ...
              key={index}
              className={`border rounded-full text-xs px-4 py-1 cursor-pointer 
                ${
                  selectedCategry === item
      ...
      {/* right col */}
      <div
        className="w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200
      min-h-96 "
      >
        <div className="flex items-center gap-3">
